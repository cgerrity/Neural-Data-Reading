function Full_CM_Table = cgg_procImportanceAnalysis_v3(InDatastore,Mdl,ClassNames,varargin)
...
% [NumDimensions,NumHierarchy]=size(Mdl);
% 
...
% % % for idx=1:NumIter
% % % 
% % %     CM_Table_Cell=cell(NumDimensions,1);
...
% % %             this_Mdl=Mdl(fdidx,:);
% % %         end
% % % [CM_Table_Cell{fdidx}] = cgg_procPredictionsFromDatastore(InDatastore_tmp,this_Mdl,ClassNames{fdidx},'DimensionNumber',fdidx);
% % %     end
% % % CM_Table{idx} = cgg_procCombinePredictions(CM_Table_Cell,'wantZeroFeatureDetector',wantZeroFeatureDetector);
% % % 
...
[~,~,CM_Table] = cgg_procTestingResults(InDatastore_tmp,Mdl,'ClassNames',ClassNames,'NumIter',NumIter);
...
VarArgIn=ReadFunctionInformation.workspace{1, 1}.varargin;
...
this_Datastore=InDatastore;

this_ChannelRemoval = ChannelRemoval{cidx,aidx};
this_ChannelRemoved=this_ChannelRemoval(1);
this_AreaRemoved=Probe_Areas{aidx};

Data_Fun=@(x) cgg_loadDataArray(x,DataWidth,StartingIDX,EndingIDX,WindowStride,this_ChannelRemoval,false,true,true,true,VarArgIn{:});
...
% % % for idx=1:NumIter
% % % 
% % %     this_IA_CM_Table_Cell=cell(NumDimensions,1);
% % %     for fdidx=1:NumDimensions
% % %         if ~wantZeroFeatureDetector
% % %             this_Mdl=Mdl{fdidx,:};
% % %         else
% % %             this_Mdl=Mdl(fdidx,:);
% % %         end
% % % [this_IA_CM_Table_Cell{fdidx}] = cgg_procPredictionsFromDatastore(this_Datastore,this_Mdl,ClassNames{fdidx},'DimensionNumber',fdidx);
% % %     end
% % % this_IA_CM_Table{idx} = cgg_procCombinePredictions(this_IA_CM_Table_Cell);
% % % 
% % % % [~,~,~,~,~,CM_Table{idx}] = cgg_procConfusionMatrixFromDatastore(InDatastore_tmp,Mdl,ClassNames,'DimensionNumber',DimensionNumber);
% % % 
% % % end
...
[~,~,this_IA_CM_Table] = cgg_procTestingResults(this_Datastore,Mdl,'ClassNames',ClassNames,'NumIter',NumIter);

% for idx=1:NumIter
% [~,~,~,~,~,this_IA_CM_Table{idx}] = cgg_procConfusionMatrixFromDatastore(InDatastore_tmp,Mdl,ClassNames,'DimensionNumber',DimensionNumber);
% end
% 
% [this_IA_CM_Table] = cgg_gatherConfusionMatrixTablesOverIterations(this_IA_CM_Table);
