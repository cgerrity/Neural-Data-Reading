...
%         TimeSize
    ...
    methods
        ...
        function [Z,mu,logSigmaSq] = predict(layer,X)
            ...
            % the layer at prediction time and output the
            % result.
            %
            % Inputs:
        ...
        function [Z,mu,logSigmaSq] = forward(layer,X)
            % [Z,mu,logSigmaSq] = forward(~,Z) Forwards input data through
            % the layer at training time and output the
            % result.
            %
            % Inputs:
            %         X - Concatenated input data where X(1:K,:) and 
