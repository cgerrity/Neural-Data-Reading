function [ValueDifferenceCategories,CategoryNames] = cgg_calcValueDifferenceCategory(Identifiers_Table,varargin)
%CGG_CALCVALUEDIFFERENCECATEGORY Summary of this function goes here
...
TargetVariableName = 'Value RL Difference';

if isempty(Identifiers_Table)
    ValueDifferenceCategories = [];
    return;
end

% Initialize the output array with zeros.
% 0 corresponds to "Not Learned" (or NaNs).
ValueDifferenceCategories = zeros(height(Identifiers_Table), 1);
...
for spidx = 1:length(uniqueSplitVariables)
    ...
    % We bin positive differences separately so "Low" means "Low Positive"
    ...
    if any(posMask)
        ...
        ValueDifferenceCategories(posMask) = posBins;
    end
    
    % --- Split 2: Negative Values (< 0) ---
    % We bin negative differences separately based on ABSOLUTE value.
    ...
    if any(negMask)
        ...
        % Use absolute values for binning so 1=Small Difference, 3=Large
        % Difference
        [negBins, ~] = cgg_calcBinsForVariable(abs(negValues), NumBins, RangeType);
        ValueDifferenceCategories(negMask) = negBins;
    end
end