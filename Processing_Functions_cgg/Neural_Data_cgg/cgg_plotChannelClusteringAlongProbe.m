...
[idx,~,~,~] = kmeans(InData,NumMeans,'Distance',InDistance,...
    'Replicates',NumReplicates,'Options',opts);

% Y = tsne(this_Data,'Algorithm','barneshut');

NumChannels=length(idx);

numGroups = length(unique(idx));
clr = hsv(numGroups);
% gscatter(Y(:,1),Y(:,2),idx,clr)

gscatter(zeros(1,NumChannels),1:NumChannels,idx,clr);
...
this_figure_save_name=[InSavePlotCFG.path filesep sprintf(InSaveName,NumMeans,InArea)];
