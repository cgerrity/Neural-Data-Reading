function [fig_plot,p_Plots,p_Error] = cgg_plotLinePlot(InData_X,InData_Y,varargin)
%CGG_PLOTLINEPLOT Summary of this function goes here
...
SameNumWindows=true;
if isnumeric(InData_Y)
    [NumExamples,NumX,NumPlots] = size(InData_Y);
elseif iscell(InData_Y)
    NumPlots = length(InData_Y);
    NumExamples=NaN(1,NumPlots);
    NumX=NaN(1,NumPlots);
    for pidx=1:NumPlots
        [NumExamples(pidx),NumX(pidx)] = size(InData_Y{pidx});
    end
    if range(NumX) ~= 0
...
%     if SameNumWindows
%         this_Time = Time_Start_Adjusted+((1:NumX)-1)*WindowStride;
%     else
%         % Insert time for different window lengths
%     end
% else
%     if SameNumWindows
%         this_Time = linspace(Time_Start,Time_End,NumX);
%     else
%         % Insert time for different window lengths
%     end
% end
...
YMin=Inf;
    ...
    for pidx=1:NumPlots
        ...
        if isnumeric(InData_X)
            this_X=InData_X(:,:,pidx);
        elseif iscell(InData_X)
            this_X=InData_X{pidx};
        end
        if isnumeric(InData_Y)
            this_Data=InData_Y(:,:,pidx);
        elseif iscell(InData_Y)
            this_Data=InData_Y{pidx};
        end
    ...
    [this_p_Plot,this_p_Error] = cgg_plotLinePlotWithShadedError(this_X,this_Data,PlotColors{pidx},'wantCI',wantCI,'SignificanceValue',SignificanceValue,'YValues_STD',YValues_STD,'CountPerSample',CountPerSample,'DataTransform',DataTransform,'ErrorMetric',this_ErrorMetric,'Error_FaceAlpha',Error_FaceAlpha,'Error_EdgeAlpha',Error_EdgeAlpha);
