...
BonferroniTestCount = NumLM*NumAreas;
BonferroniSignificance = SignificanceValue/BonferroniTestCount;
...
for aidx = 1:NumAreas
    ...
    Plot_Bar{lidx}(aidx) = PlotData.HomogeneityIndex_Correlation;
        if WantCI
        this_CI = PlotData.HomogeneityIndex_Correlation_CI;
        if WantBonferroni
            this_STE = PlotData.HomogeneityIndex_Correlation_STE;
            this_SampleCount = PlotData.NumData;
            this_ts = tinv(1-(BonferroniSignificance/2),this_SampleCount-1);
            this_CI = this_ts.*this_STE;
        end
        PlotError{lidx}(aidx) = this_CI;
        elseif WantSTD
        PlotError{lidx}(aidx) = PlotData.HomogeneityIndex_Correlation_STD;
        else
    ...
    Plot_Bar{lidx}(aidx) = PlotData.HomogeneityIndex;
        if WantCI
        this_CI = PlotData.HomogeneityIndex_CI;
        if WantBonferroni
            this_STE = PlotData.HomogeneityIndex_STE;
            this_SampleCount = PlotData.NumData;
            this_ts = tinv(1-(BonferroniSignificance/2),this_SampleCount-1);
            this_CI = this_ts.*this_STE;
        end
        PlotError{lidx}(aidx) = this_CI;
        elseif WantSTD
        PlotError{lidx}(aidx) = PlotData.HomogeneityIndex_STD;
        else
