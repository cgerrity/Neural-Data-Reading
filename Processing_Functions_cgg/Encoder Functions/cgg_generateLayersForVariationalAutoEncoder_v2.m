...
Layers_Custom= dlnetwork(layerGraph([ ...
    sequenceInputLayer(InputSize,"Name","sequence_Encoder"), ...
    gruLayer(HiddenSizes(1),"Name","Layer_To_Replace","OutputMode","sequence"), ...
    fullyConnectedLayer(prod(InputSize,"all"),"Name","fc_Decoder_Out"), ...
...
for stidx=NumStacks:-1:1
    ...
    this_Encoder_GRUName=sprintf("gru_Encoder_%d",stidx);
    ...
    this_Decoder_GRUName=sprintf("gru_Decoder_%d",stidx);
    ...
    this_Layer_Encoder = [
        gruLayer(this_HiddenSize,"Name",this_Encoder_GRUName,"OutputMode","sequence")
        layerNormalizationLayer('Name',this_Encoder_NormalizationName)
        dropoutLayer(DropoutPercent,'Name',this_Encoder_DropOutName)];
    this_Layer_Decoder = [
        gruLayer(this_HiddenSize,"Name",this_Decoder_GRUName,"OutputMode","sequence")
        layerNormalizationLayer('Name',this_Decoder_NormalizationName)
        dropoutLayer(DropoutPercent,'Name',this_Decoder_DropOutName)];
    % if stidx==NumStacks
    %     this_Layer_Decoder=[];
    % end
