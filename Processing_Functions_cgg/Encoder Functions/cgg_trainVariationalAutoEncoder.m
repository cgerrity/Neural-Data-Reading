function cgg_trainVariationalAutoEncoder(InDataStore,DataStore_Testing,SessionsList,cfg_Encoder,ExtraSaveTerm,Encoding_Dir)
...
BaseSavePathNameExt=[Encoding_Dir filesep 'VariationalAutoEncoder_Base' ExtraSaveTerm '.mat'];
...
%% TESTING
% 
[~,Layers_Custom_TEST] = cgg_generateLayersForVariationalAutoEncoder_v2(DataSize,[100,20],NumWindows,DataFormat_Reshape);

InputNet_TEST= initialize(Layers_Custom_TEST);
% InputNet_TEST= Layers_Custom_TEST;

Layer_Test='None';

if ~strcmp(Layer_Test,'None')
InputNet_TEST = initialize(removeLayers(InputNet_TEST,{InputNet_TEST.Layers(Layer_Test:end).Name}));
else
InputNet_TEST = initialize(InputNet_TEST);
end

% NameLastLayer=InputNet_TEST.Layers(end).Name;
% 
% NormalizationLayer_TEST=layerNormalizationLayer('Name','NormalizationLayer');
% 
% InputNet_TEST = addLayers(InputNet_TEST,NormalizationLayer_TEST);
% InputNet_TEST=connectLayers(InputNet_TEST,NameLastLayer,'NormalizationLayer');
...
% 
OutputNames_TEST=["reshape_Decoder","SamplingLayer/mean","SamplingLayer/log-variance"];
OutputNames_TEST=InputNet_TEST.OutputNames;
OutputExample=cell(1,length(OutputNames_TEST));
[OutputExample{:}]=forward(InputNet_TEST,X_TEST,Outputs=OutputNames_TEST);
% 
% [Output_TEST_mu,Output_TEST_sig,Output_TEST_Z]=forward(InputNet,X_TEST);

% [Output_TEST]=forward(InputNet_TEST,X_TEST);
% 
% size(Output_TEST_Z)
% size(Output_TEST_mu)
...
for seidx=1:NumSessions
    ...
    SessionSavePathNameExt=[Encoding_Dir filesep 'VariationalAutoEncoder_Session' '_' this_SessionName ExtraSaveTerm '.mat'];
...
EncoderSessionSavePathNameExt=[Encoding_Dir filesep 'VariationalEncoder_Session' '_' this_SessionName ExtraSaveTerm '.mat'];
...
FullTuningSavePathNameExt=sprintf([Encoding_Dir filesep 'VariationalFullTuning' ExtraSaveTerm '.mat']);
