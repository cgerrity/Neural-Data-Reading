...
for fidx = 1:length(FilterColumn_All)
    ...
    this_WantAnalysis = WantAnalysis;
    % if strcmp(FilterColumn,'Trials From Learning Point Category')
    %     WantUseNullTable = false;
    %     this_WantAnalysis = false;
    % end
    fprintf('### Running Trial Filter for %s!\n',string(join(FilterColumn)));
    [FullTable,cfg] = cgg_getResultsPlotsParametersNetwork(EpochName,'FilterColumn',FilterColumn,'WantAnalysis',this_WantAnalysis,'WantResults',WantResults,'WantUseNullTable',WantUseNullTable,'WantLabelClassFilter',WantLabelClassFilter);
end