function [data] = ft_resampledata_bv(cfg, data)
...
%BV: check that trial data is in the right format
if isvector(data.trial{1}) && ~isrow(data.trial{1})
    for ii=1:numel(data.trial)
        data.trial{ii} = data.trial{ii}';
    end
...
if ~iscell(cfg.time); cfg.time = {cfg.time}; end
