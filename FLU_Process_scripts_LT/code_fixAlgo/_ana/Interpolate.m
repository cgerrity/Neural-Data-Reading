function [interpolatedData, numInterps] = Interpolate(inputData, timeStamps, maxNanTime, timeWindow, fsample)
...
singleNan = findPattern(isnan(inputData), [0 1 0]) + 1;
for i = 1:length(singleNan)
    ...
    timeWindowRows = max(1,ceil(singleNan(i) - sampleWindow/2)) : min(length(inputData), floor(singleNan(i) + sampleWindow/2));
    ...
    if sum(nanRows) <= maxNanSamples %if there is less than the maximum amount of bad data, replace the single nan missing data point via interpolation
        interpolatedData(singleNan(i)) = interp1(goodRows, inputData(goodRows), singleNan(i), 'pchip');
        numInterps(1) = numInterps(1) + 1;
    end
