...
while saccadesExist ==1
    ...
    %VelThresh = AccThresh;
    isSac = nan(size(eyeData,1),2);

        

    %seperatley for x and y
    for iax=1:2

        %seperate threshold for increase and decrease
        accThresh = nan(size(eyeData,1),2);
        for ii=1:2
            for it=1:size(trialIndices,1)
                ...
                accThresh(st:fn,ii) = c * iterativeThreshold(a,10000,1,lambdaAcc);
            end

            %find threshold crossings
            tmp = size(accThresh,1);
            tmp(Acc(:,iax) > accThresh(:,1)) = 1;
            tmp(Acc(:,iax) < accThresh(:,2)) = -1;
...
%    1.497383464939551   1.497383465086199
    ...
    toi_all=1.0e+09 * [  1.498593380291482   1.498593380338150
   1.498593380351548   1.498593380378142
   1.498593788460214   1.498593788523520
   1.498593788533539   1.498593788560205
   1.498594086629679   1.498594086682997
   1.498594086689665   1.498594086749656
   1.498594931025864   1.498594931052534
   1.498594931069300   1.498594931099232
   1.498594958874780   1.498594958914885
   1.498594958928224   1.498594958981551
   1.498595508407874   1.498595508471261
   1.498595508477919   1.498595508557902];
