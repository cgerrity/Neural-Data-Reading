function t = readtable2018(filename,varargin)
%READTABLE Create a table by reading from a file.
%   Use the READTABLE function to create a table by reading column-oriented data from
%   a file. READTABLE can automatically determine the file format from its extension
%   as described below.
%
%   T = READTABLE(FILENAME) creates a table by reading from the file FILENAME, and
%   determines the file format from its extension. The extension must be one of those
%   listed below.
%
%   T = READTABLE(FILENAME,'FileType',FILETYPE) specifies the file type, where
%   FILETYPE is one of 'text' or 'spreadsheet'.
%
%   T = READTABLE(FILENAME,OPTS) creates a table from the file FILENAME using the
%   supplied ImportOptions OPTS. OPTS specifies variable names, selected variable
%   names, variable types, and other information regarding the location of the data.
%
...
%                          spreadsheet in basic mode. Basic mode is the default for
%                          systems without Excel for Windows installed. Basic mode
%                          may read files where there may be live updates (e.g.
%                          formula evalauation or plugins) to the data differently.
%                          If your files do not have this requirement, basic mode can
%                          be used.
%
