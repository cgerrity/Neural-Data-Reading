...
if true
    ...
    % Folder for the specific session data
...
preproc_config_rec.channel = ...
  ft_channelselection( chanmap_rec_cooked(probe_selection), rechdr.label, {} );
...
if want_artifact_rejection
cfg_visual = [];
cfg_visual.method = 'summary';
% cfg_visual.ylim = [-1e-12 1e-12];
recdata_wideband_rejected = ft_rejectvisual_cgg(cfg_visual, recdata_wideband);
    
end
%% Data Cutting with start and end point

this_data_struct=recdata_activity;
sel_cut_start_event='StimOn';
sel_cut_end_event='FixObjectEnd';

NumTrials=length(this_data_struct.trial);

All_Trials_Data=cell(1,NumTrials);
All_Trials_Time=cell(1,NumTrials);
...
this_events_codes=trialcodes_each{1, sel_trial};
this_events_codes_codeLabel=trialcodes_each{1, sel_trial}.codeLabel;
this_events_codes_recTime=trialcodes_each{1, sel_trial}.recTime;

this_time_series=this_data_struct.time{sel_trial};
this_time_series_absolute=this_time_series+time_offsets(sel_trial);

sel_cut_start_event_index=strcmp(this_events_codes.codeLabel,sel_cut_start_event);
sel_cut_end_event_index=strcmp(this_events_codes.codeLabel,sel_cut_end_event);

for lidx=1:length(sel_cut_end_event_index)
    if sel_cut_end_event_index(lidx)==1
        if sum(sel_cut_end_event_index(lidx+1:end))>0
            sel_cut_end_event_index(lidx)=0;
        end
    end
end

sel_cut_start_time=this_events_codes_recTime(sel_cut_start_event_index);
sel_cut_end_time=this_events_codes_recTime(sel_cut_end_event_index);

[~,sel_cut_start_index]=min(abs(this_time_series_absolute-sel_cut_start_time));
[~,sel_cut_end_index]=min(abs(this_time_series_absolute-sel_cut_end_time));

this_trial=this_data_struct.trial{1, sel_trial};

this_cut_trial=this_trial(:,sel_cut_start_index:sel_cut_end_index);
this_cut_time=this_time_series(:,sel_cut_start_index:sel_cut_end_index);

All_Trials_Data{tidx}=this_cut_trial;
All_Trials_Time{tidx}=this_cut_time;

end

% plot(this_cut_time,this_cut_trial)

%% Data Cutting around specified point using Event Codes

this_data_struct=recdata_activity;
% sel_cut_single_event={'Unrewarded','Rewarded','NoChoice'};
sel_cut_single_event={'TrlStart'};

sel_cut_time_window_before=0;
sel_cut_time_window_after=0.5;

NumTrials=length(this_data_struct.trial);

All_Trials_Data_Single=[];
All_Trials_Time_Single=[];
...
sel_cut_event_index=0;
for sidx=1:length(sel_cut_single_event)
    
sel_cut_event_index=sel_cut_event_index+strcmp(this_events_codes.codeLabel,sel_cut_single_event{sidx});
 
end

if sum(sel_cut_event_index)==0
    disp(tidx)
end

sel_cut_event_index=sel_cut_event_index==1;
 
sel_cut_event_index_check=strcmp(this_events_codes.codeLabel,'FixObjectEnd');


sel_cut_time=this_events_codes_recTime(sel_cut_event_index);
sel_cut_time_check=this_events_codes_recTime(sel_cut_event_index_check);

sel_cut_start_time=sel_cut_time-sel_cut_time_window_before;
sel_cut_end_time=sel_cut_time+sel_cut_time_window_after;

this_trial=this_data_struct.trial{1, sel_trial};

for eidx=1:length(sel_cut_time)
    this_sel_cut_start_time=sel_cut_start_time(eidx);
    this_sel_cut_end_time=sel_cut_end_time(eidx);
    this_sel_cut_time=sel_cut_time(eidx);
    
    this_sel_cut_check=sel_cut_time_check-this_sel_cut_time;
    this_sel_cut_check=this_sel_cut_check(this_sel_cut_check>0);
    
%     if min(this_sel_cut_check)>

[~,sel_cut_start_index]=min(abs(this_time_series_absolute-this_sel_cut_start_time));
[~,sel_cut_end_index]=min(abs(this_time_series_absolute-this_sel_cut_end_time));

this_cut_trial=this_trial(:,sel_cut_start_index:sel_cut_end_index);
this_cut_time=this_time_series_absolute(:,sel_cut_start_index:sel_cut_end_index)-this_sel_cut_time;

NumCuts=NumCuts+1;

All_Trials_Data_Single(NumCuts,:)=this_cut_trial;
All_Trials_Time_Single(NumCuts,:)=this_cut_time;

end

end

% All_Trials_Data_Single_Mean=mean(All_Trials_Data_Single,1);
% All_Trials_Data_Single_STD=std(All_Trials_Data_Single,0,1)/sqrt(NumTrials);
% All_Trials_Time_Single_Mean=mean(All_Trials_Time_Single,1);
% 
% 
% plot(All_Trials_Time_Single_Mean,All_Trials_Data_Single_Mean) 

%% Data Cutting around specified point using Frame Data

this_data_struct=recdata_activity;
% sel_cut_single_event={'Unrewarded','Rewarded','NoChoice'};
sel_cut_single_event={'SelectObject'};

sel_cut_time_window_before=1;
sel_cut_time_window_after=1.5;

NumTrials=length(this_data_struct.trial);

All_Trials_Data_Frame=[];
All_Trials_Time_Frame=[];
...
this_frame_data=gameframedata;

this_trial_frame_data=this_frame_data(this_frame_data.TrialCounter==tidx,:);

sel_cut_frame_index=strcmp(this_trial_frame_data.TrialEpoch,sel_cut_single_event{end});

this_frame=this_trial_frame_data(sel_cut_frame_index,:);
...
sel_cut_time=this_frame.recTime;
...
All_Trials_Data_Frame(NumCuts,:)=this_cut_trial;
All_Trials_Time_Frame(NumCuts,:)=this_cut_time;
...
%% Data Saving
% dataFolder=['/Volumes/gerritcg''','s home/Data_Neural_gerritcg/Fr_Probe_02_22-05-09_009_01/Data_Processing'];
% 
% data_variables_names={'evcodedefs',

% save('Fr_Probe_02_22-05-09_009_01_processed.mat',names)
